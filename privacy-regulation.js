...
const completedLessons = new Set(JSON.parse(localStorage.getItem("privacyLessons") || "[]"));
...
completeLessonButtons.forEach((button) => button.addEventListener("click", () => {
  ...
  localStorage.setItem("privacyLessons", JSON.stringify([...completedLessons]));
  updateProgress();
...
document.querySelector(".submit-quiz").addEventListener("click", () => {
  ...
  if (correct === fields.length) {
    localStorage.setItem("privacyComplete", "true");
